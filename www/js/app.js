...
mapaKniznicApp.controller('mapCtrl', function($scope, libraries, geoutils, maputils) {
  var map = L.map('map')
  ...
  (libraries.getAll().features).forEach(function(library, i) {
    var id = library.id
    if (id.indexOf('node') == '0') {
      var lon = library.geometry.coordinates[0]
      var lat = library.geometry.coordinates[1]
      maputils.createMarker(map, library, lat, lon)
    } else if (id.indexOf('way') == '0') {
      var latlon = geoutils.avgLatLon(library.geometry.coordinates[0])
      maputils.createMarker(map, library, latlon[0], latlon[1])
    }
  })
})